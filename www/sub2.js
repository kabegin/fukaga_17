...
function tasu(){
   ...
   //alert(add);
    var k = '<input type="button" value="前のページへ戻る" onclick="location.reload(true)"><table class=thin><caption style="background-color:green">個別予定表</caption>';
     k += '<ons-scroller><ons-list><ons-listheader bgcolor="#3DF74A">年月日<bgcolor="#3DF74A">曜< bgcolor="#3DF74A">予定</ons-list-header>'; 
     do{   
      day = new Date(hi(add));//iには'2007/5/5':'PHP攻略完了'の形式の前の方の文字列が入っている。
      w = week[day.getDay()];//曜日を取得
        var aq=parseInt(deban+maime);
    ...
    if (job==undefined || deban=="7"){job="休み";}//or maime="7"
       k += c+'<th>' + hi(add) + '</th><td>' + w + '</td><td>' +deban+maime+" "+job +'</td></tr></ons-list-item>';
...
for(var i=1;i<10;i++){
    tdStr2 = $("tr").eq(1+i).children().eq(2).text().substr(0,3);
    tdStr1 = $("tr").eq(1+i).children().eq(0).text();
    obj[tdStr1]=tdStr2;
}
localStorage.setItem('obj',JSON.stringify(obj));
var d=JSON.parse(localStorage.getItem("obj"));
alert(d["2016/10/6"]);
for(var key in obj){
    alert(key+" : "+obj[key]);
}
